...
// podSandboxConfigTypeURL is the typeurl type URL containerd's CRI plugin
// uses for a marshaled k8s.io/cri-api runtime.v1.PodSandboxConfig (verified
// against containerd's RunPodSandbox, which passes the CRI-supplied
// *runtime.PodSandboxConfig to sandbox.WithOptions, ultimately reaching
// CreateSandboxRequest.Options as typeurl.MarshalAny(config) -- typeurl's
// bare-full-name convention, not a URL with a "type.googleapis.com/"
// scheme, for a type with no explicit typeurl.Register call). Checked here
// as a plain string rather than via typeurl's registry-based dispatch:
// types.CRIPodConfig is a distinct Go type from the real upstream message,
// deliberately not registered with typeurl under this name (typeurl.Register
// only panics if the same Go type is later registered under a different
// path -- it does not detect two different types sharing one path -- so it
// would not actually guard against a future k8s.io/cri-api reintroduction
// anyway; decoding goes straight through proto.Unmarshal below instead).
const podSandboxConfigTypeURL = "runtime.v1.PodSandboxConfig"

// decodePodSandboxConfig unmarshals a sandbox's CreateSandboxRequest.Options
// into a types.CRIPodConfig. opts is exactly what SandboxService.Options()
...
// but was not a PodSandboxConfig (wrong type URL) or failed to decode as
// one; callers should treat that as non-fatal too (log and continue
// without pod config) since a shim must never fail Task.Create over an
// optional, best-effort feature.
func decodePodSandboxConfig(opts *anypb.Any) (*types.CRIPodConfig, error) {
	if opts == nil {
		return nil, nil
	}
	if opts.GetTypeUrl() != podSandboxConfigTypeURL {
		return nil, fmt.Errorf("unexpected sandbox options type %q, want %q", opts.GetTypeUrl(), podSandboxConfigTypeURL)
	}
	cfg := &types.CRIPodConfig{}
	if err := proto.Unmarshal(opts.GetValue(), cfg); err != nil {
