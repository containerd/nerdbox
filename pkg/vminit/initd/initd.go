...
func systemInit(ctx context.Context, config Config, shutdownSvc shutdown.Service) error {
	...
	//
	// Only ever raise the limit, never lower it: read the current soft/hard
	// limits first and leave them untouched if either is already at or above
	// the target, so we never clobber a higher value set by the guest kernel
	// or anything that ran before us.
	const wantNofile = 65536
	var nofileLimit unix.Rlimit
	if err := unix.Getrlimit(unix.RLIMIT_NOFILE, &nofileLimit); err != nil {
		log.G(ctx).WithError(err).Warn("failed to read RLIMIT_NOFILE; leaving it unchanged")
	} else if nofileLimit.Cur < wantNofile || nofileLimit.Max < wantNofile {
		if nofileLimit.Cur < wantNofile {
			nofileLimit.Cur = wantNofile
		}
		if nofileLimit.Max < wantNofile {
			nofileLimit.Max = wantNofile
		}
