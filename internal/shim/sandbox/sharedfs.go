...
// separator. containerID arrives over the shim-v2 TTRPC API from
// Task.Create and is never trusted before being used to build a
// filesystem path. Mirrors sharedresources.validateID's rules on the
// guest side, which the same untrusted-ID-over-RPC reasoning applies to.
func validateContainerID(id string) error {
	if id == "" {
		return fmt.Errorf("container id is required")
	}
	if id == "." || id == ".." || strings.ContainsRune(id, '/') || strings.ContainsRune(id, os.PathSeparator) || strings.ContainsRune(id, 0) {
