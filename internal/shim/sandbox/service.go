...
func (s *SandboxService) StartSandbox(ctx context.Context, req *sandboxAPI.StartSandboxRequest) (*sandboxAPI.StartSandboxResponse, error) {
	...
	// A stopped sandbox cannot be restarted: StopSandbox has already
	// released s.networkSandbox (set to nil below) and closed s.exitCh,
	// neither of which this function knows how to recreate. Restarting a
	// stopped sandbox is not part of the shim-v2 sandbox lifecycle
	// contract anyway — once stopped, a sandbox is torn down, not reused.
	if s.state == sandboxStateStopped {
		return nil, errgrpc.ToGRPC(fmt.Errorf("sandbox already stopped: %w", errdefs.ErrFailedPrecondition))
	}
...
// StopSandbox.
//
// Propagates a StopSandbox failure rather than reporting success
// unconditionally: a caller that only sees success has no reason to retry,
// so a failed VM shutdown, network sandbox release, or mount cleanup would
// otherwise go unnoticed and unretried, potentially leaking all three. A
// retry is safe: StopSandbox's own state check makes a second call a no-op
// once it has actually succeeded, and re-running it after a failure re-runs
// only the steps that did not complete (SharedFS.UnshareAll is safe to call
// again — see Unshare's own idempotency — and the underlying VM's Shutdown
// guards against being torn down twice).
func (s *SandboxService) ShutdownSandbox(ctx context.Context, req *sandboxAPI.ShutdownSandboxRequest) (*sandboxAPI.ShutdownSandboxResponse, error) {
	...
	if _, err := s.StopSandbox(ctx, &sandboxAPI.StopSandboxRequest{SandboxID: req.SandboxID}); err != nil {
		// Not re-wrapped: StopSandbox already returns an errgrpc.ToGRPC
		// error carrying a gRPC status; wrapping it again risks losing
		// that status across the TTRPC boundary depending on how the
		// transport extracts it.
		return nil, err
	}
