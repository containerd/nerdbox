...
func (s *service) createSandboxedContainer(ctx context.Context, r *taskAPI.CreateTaskRequest) (_ *taskAPI.CreateTaskResponse, err error) {
	...
	podCfg, err := decodePodSandboxConfig(s.svc.Options())
	if err != nil {
	...
	b, err := bundle.Load(ctx, r.Bundle,
		...
		func(ctx context.Context, b *bundle.Bundle) error {
			// fallbackToHostRC (copy the host's own resolv.conf) only makes
			// sense when this container has no dedicated NIC and so relies
			// on TSI for connectivity: with a NIC, the container has its
			// own real guest-side network stack and should get a
			// network-appropriate resolver, not the host's. ctrNetCfg is
			// already populated here — ctrNetCfg.fromBundle runs earlier in
			// this same transformer chain.
			return addResolvConf(ctx, b, len(ctrNetCfg.Networks) == 0, podCfg.GetDnsConfig())
		},
		func(ctx context.Context, b *bundle.Bundle) error {
		...
		func(ctx context.Context, b *bundle.Bundle) error {
			return sanitizeNamespaces(ctx, b, len(ctrNetCfg.Networks) > 0, sharedRes.get)
		},
...
func (s *service) Delete(ctx context.Context, r *taskAPI.DeleteRequest) (*taskAPI.DeleteResponse, error) {
	...
	if err == nil {
		...
		// Unshare the container's rootfs from the shared filesystem. This
		// unmounts the host-side overlay/bind and removes the container
		// subtree from <sandbox-state>/containers/<id>. Only set for the
		// init process on the sandboxed path.
		if sharedFSID != "" && s.svc != nil {
			if fs := s.svc.FS(); fs != nil {
				if err := fs.Unshare(ctx, sharedFSID); err != nil {
					log.G(ctx).WithError(err).WithField("id", sharedFSID).Warn("failed to unshare container rootfs on delete")
				}
			}
		}
	}
