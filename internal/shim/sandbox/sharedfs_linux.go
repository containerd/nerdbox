...
func (s *SharedFS) unshare(ctx context.Context, containerID string) error {
	...
	for i := len(mountPts) - 1; i >= 0; i-- {
		...
		if err := mount.UnmountAll(pt, unix.MNT_DETACH); err != nil {
			...
			// Keep this target so a later retry (another Unshare or
			// UnshareAll call) still knows about it. Without this, the
			// bookkeeping delete below would drop it permanently after
			// this one failed attempt, and it would never be retried.
			remaining = append(remaining, pt)
		}
	}

	// Replace, rather than unconditionally delete, so a failed unmount's
	// target survives for a later retry. remaining was built by appending
	// while iterating mountPts in reverse (deepest/outermost first), so it
	// is in the opposite order from mountPts's own outermost-last
	// convention (see shareRootfs) — reverse it back before storing so a
	// later retry's own reverse iteration unmounts outermost (e.g.
	// hostRootfs) first again, not last.
	s.mu.Lock()
	if len(remaining) > 0 {
		slices.Reverse(remaining)
		s.mounts[containerID] = remaining
	} else {
		delete(s.mounts, containerID)
	}
	s.mu.Unlock()

	// Only remove the container subtree once every mount under it is
	// confirmed gone: with a mount still active, RemoveAll could remove
	// entries from a stale, about-to-be-detached view of the directory
	// rather than its real (post-unmount) contents, or simply fail
	// outright on a busy mount point — neither of which is better than
	// leaving the directory for the next retry to clean up alongside the
	// mount it still needs to unmount.
	if len(remaining) == 0 {
